...
impl<O,V> From<O> for ObserverNotifiedErased<O,V> where O: ObserverNotified<V> {
...
impl<O,V> AsRef<O> for ObserverNotifiedErased<O,V> where O: ObserverNotified<V> {
    fn as_ref(&self) -> &O {
        &self.0
    }
}

impl<O,V> AsMut<O> for ObserverNotifiedErased<O,V> where O: ObserverNotified<V> {
    fn as_mut(&mut self) -> &mut O {
        &mut self.0
    }
}

impl<O,V> Deref for ObserverNotifiedErased<O,V> where O: ObserverNotified<V> {
    type Target = O;
...
impl<O,V> DerefMut for ObserverNotifiedErased<O,V> where O: ObserverNotified<V> {
